...
from v1.script_1_populate_database import download
...
class ScraperTest(unittest.TestCase):
    def test_writes_raw_and_categorized_indexes(self):
        ...
        listings = [
            {
                "pageData": [{"id": "w1", "name": "Sword"}],
                "pageCount": 1,
                "facetDistribution": {"grade": {"1": 1}},
            },
            {
                "pageData": [{"id": "a1", "name": "Helmet"}],
                "pageCount": 1,
                "facetDistribution": {"grade": {"1": 1}},
            },
            {
                "pageData": [{"id": "g1", "name": "Blue Agate"}],
                "pageCount": 1,
                "facetDistribution": {"grade": {"0": 1}},
            },
        ]
        details = [
            {
        ...
        session.get.side_effect = [
            Mock(**{"raise_for_status.return_value": None, "json.return_value": value})
            for value in values
        ]
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            download(session, db)
            self.assertEqual(
                json.loads((db / "index.json").read_text()),
                {"w1": "Sword", "a1": "Helmet", "g1": "Blue Agate"},
            )
